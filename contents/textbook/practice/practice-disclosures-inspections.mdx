...
A seller inherited a house, never occupied it, and believes that nonoccupancy removes every disclosure duty. That conclusion skips the legal test. Determine whether the particular transfer fits a statutory exemption, identify the remaining disclosures, and address known material information regardless of whether a familiar form is required. The seller's lack of firsthand knowledge can affect the answers, but it is not a license to answer no to conditions the seller has never investigated or to suppress an existing report.

Suppose the inherited property comes with a contractor's letter describing recurring foundation movement. Even if a specific form exemption applies, the letter may contain material information that must be addressed. An agent should not advise destroying it because the seller did not commission it. The buyer's ability to order another inspection does not erase the significance of information already available to the seller or agent.

The same approach applies to an as-is sale. One issue is whether the seller agrees to repair; another is whether the buyer receives accurate information. A buyer can knowingly accept a disclosed condition at an agreed price. That is different from being induced to accept an undisclosed condition through a false answer or a concealed report.
...
A covered buyer receives a materially amended TDS after offer execution through agreed electronic delivery. The statutory five-day termination opportunity must be considered even if the contract's inspection contingency has a different deadline. Neither clock automatically replaces the other. Determine whether the required sections were completed and delivered, the method of delivery, and the event that triggered each right.
