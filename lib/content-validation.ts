...
export function validateContent(lessons: Lesson[], questions: Question[]) {
  ...
  for (const lesson of lessons) {
    ...
    if (lesson.body.split(/\s+/).length < 950) errors.push(`Chapter too short: ${lesson.slug}`)
    if (!lesson.slug.startsWith(`${lesson.domain}-`))
      errors.push(`Domain/slug mismatch: ${lesson.slug}`)
    if (questions.filter((q) => q.lessonSlug === lesson.slug).length < 3)
      errors.push(`Fewer than three questions: ${lesson.slug}`)
  }
